#include <assert.h>  /* for static_assert in the generated Axon model header */
/*
...
#include "../dmic.h"
#include "kws.h"
#ifndef KWS_MODEL_HEADER
#error "KWS_MODEL_HEADER must be set by zephyr/CMakeLists.txt"
#endif
...
/* Classes exported by solution 95649. Keep this order in sync with
 * nrf_edgeai_user_model_labels.h from Nordic Edge AI Lab. */
enum keyword_class {
	KEYWORD_OTHER,
	KEYWORD_SILENCE,
	KEYWORD_NO,
...
static const struct keyword_detection_ctx keyword_detection_ctxs[] = {
	[KEYWORD_OTHER] = {.name = "OTHER", .threshold = 0.9f, .num_in_row = 22},
	[KEYWORD_SILENCE] = {.name = "Silence", .threshold = 0.9f, .num_in_row = 22},
	[KEYWORD_NO] = {.name = "No", .threshold = 0.9f, .num_in_row = 22},
	[KEYWORD_OK] = {.name = "Ok", .threshold = 0.9f, .num_in_row = 22},
	[KEYWORD_OPUS] = {.name = "Opus", .threshold = 0.9f, .num_in_row = 22},
...
int kws_init(void)
{
	/* Solution 95649: OTHER, SILENCE, no, ok, opus, stop, yes. */
	kws_model = nrf_edgeai_user_model_95649();
...
static void kws_postprocess(struct kws_prediction *const prediction)
{
	...
	if (predicted_class == KEYWORD_SILENCE || predicted_class == KEYWORD_OTHER) {
