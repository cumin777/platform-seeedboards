...
/* The board's green LED is independent of the SD card interface.  Keep it
 * blinking so that a stalled card initialization is distinguishable from an
 * application that did not start. */
#define STATUS_LED_NODE DT_ALIAS(led2)
...
static void status_blink_thread(void *arg1, void *arg2, void *arg3)
{
	ARG_UNUSED(arg1);
	ARG_UNUSED(arg2);
	ARG_UNUSED(arg3);
...
K_THREAD_DEFINE(sd_status_blink, 512, status_blink_thread, NULL, NULL, NULL,
		7, 0, 0);
...
int main(void)
{
	LOG_INF("USB CDC console ready; green status LED is blinking");
	LOG_INF("========================================");
