 ...
 *
 * XIAO STM32C5 IMU six-axis hello sample.
 *
 * Runtime state:
 * - LSM6DS3TR-C accel + gyro sampling over I2C2.
 * - External flash CS# PB10 driven high, so the flash is deselected/standby.
 * - CAN transceiver standby pin PB14 driven low, so the transceiver is enabled.
 * - HSE is kept on.
 * - User LED toggles every second.
 */
...
static int configure_board_standby_pins(void)
{
	...
	if (ret < 0) {
		printk("Failed to drive CAN_STB inactive: %d\n", ret);
		return ret;
	}
...
int main(void)
{
	...
	printk("\n");
	printk("Hello, XIAO STM32C5\n");
	printk("State: flash CS# PB10 high, CAN_STB PB14 low/enabled, HSE on, IMU 6-axis %d Hz\n",
	       IMU_ODR_HZ);
	printk("IMU: LSM6DS3TR-C on I2C2 PB3/PB4, address 0x6a\n");
	printk("HSE before enable: %s\n", hse_ready() ? "ready/on" : "off/not-ready");

	ret = ensure_hse_on();
	printk("HSE enable ret=%d, after: %s\n",
