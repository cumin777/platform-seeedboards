 ...
 *
 * XIAO STM32C5 low-power IMU power-down sample.
 */
...
static void print_imu_registers(const char *prefix)
{
	...
	(void)read_imu_reg(LSM6DSL_REG_TAP_CFG, &tap_cfg);
	(void)read_imu_reg(LSM6DSL_REG_MD1_CFG, &md1_cfg);

	printk("%s WHO_AM_I=0x%02x%s CTRL1_XL=0x%02x CTRL2_G=0x%02x TAP_CFG=0x%02x MD1_CFG=0x%02x\n",
	       prefix,
	       who_am_i,
...
static int configure_imu_power_down(void)
{
	int ret;

	ret = write_imu_reg(LSM6DSL_REG_MD1_CFG, 0x00);
	if (ret < 0) {
		return ret;
	}
	...
	ret = write_imu_reg(LSM6DSL_REG_TAP_CFG, 0x00);
	if (ret < 0) {
		return ret;
	}

	ret = write_imu_reg(LSM6DSL_REG_WAKE_UP_THS, 0x00);
	if (ret < 0) {
		return ret;
	}

	ret = write_imu_reg(LSM6DSL_REG_WAKE_UP_DUR, 0x00);
	if (ret < 0) {
		return ret;
	}

	ret = write_imu_reg(LSM6DSL_REG_CTRL2_G, 0x00);
	if (ret < 0) {
		return ret;
	}

	return write_imu_reg(LSM6DSL_REG_CTRL1_XL, 0x00);
}
...
int main(void)
{
	...
	printk("\n");
	printk("XIAO STM32C5 IMU power-down sample\n");
	printk("State: flash CS# high, CAN standby, HSE off, IMU accel+gyro power-down\n");
	...
	ret = configure_imu_power_down();
	if (ret < 0) {
		printk("Failed to configure IMU power-down mode: %d\n", ret);
		return 0;
	}
	...
	for (;;) {
		printk("IMU remains in power-down: CTRL1_XL=0x00 CTRL2_G=0x00\n");
		k_sleep(K_SECONDS(5));
	}
