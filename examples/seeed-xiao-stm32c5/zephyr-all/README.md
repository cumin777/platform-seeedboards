...
- CAN FD controller self-test at 1 Mbps arbitration / 8 Mbps data, using an internal FD+BRS loopback frame before switching back to normal CAN mode
...
- `CONFIG_I2C`, `CONFIG_SPI`, `CONFIG_CAN`, and `CONFIG_CAN_FD_MODE` are explicitly enabled. At boot, the sample runs one internal FDCAN2 loopback test with a 16-byte CAN FD+BRS frame, then restarts FDCAN2 in normal FD mode. No external CAN bus is required for this self-test.
- In the USB report, `CAN loopback ... frame=OK` and a normal-mode `CAN status ... state=error-active` confirm the controller/timing setup. The report also prints the CAN kernel clock and the selected 8 Mbps data-phase timing.
